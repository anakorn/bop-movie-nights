...
const Poll: React.FC = () => {
	...
	const activePollOptions =
		activePoll && pollOptionOrderMap[activePoll.id]
			? pollOptionOrderMap[activePoll.id].map(
					imdbId =>
						activePoll.options.find(
							option => option.imdbId === imdbId
						) as PollOption
			  )
			: [];

	const imgs = activePollOptions
	...
	const currentMovie = useMemo(
		() => activePollOption && getMovieById(activePollOption.imdbId),
	...
	const activeIndex = useMemo(() => {
		...
		return activePollOptions.findIndex(
			o => o.imdbId === activePollOption.imdbId
		);
